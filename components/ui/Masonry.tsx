...
const Masonry: React.FC<MasonryProps> = ({
    ...
    const grid = useMemo<GridItem[]>(() => {
        ...
        return items.map(child => {
            ...
            const height = child.height / 2; // Arbitrary height scaling from original code
    ...
    return (
        <div ref={containerRef} className="relative w-full min-h-[500px]" style={{ height: Math.max(...grid.map(i => i.y + i.h)) + 50 }}>
            {grid.map(item => (
                <div
                    key={item.id}
                    data-key={item.id}
                ...
                >
                    <div
                        className="relative w-full h-full bg-cover bg-center rounded-[10px] shadow-lg overflow-hidden group"
