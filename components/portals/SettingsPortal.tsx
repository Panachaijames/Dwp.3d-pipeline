...
export const SettingsPortal: React.FC = () => {
    ...
    const fetchUsers = async () => {
        try {
            ...
            setUsers(nextUsers);
            setDraftRoles(
                nextUsers.reduce<Record<string, UserRole>>((accumulator, entry) => {
                    accumulator[entry.email] = entry.role;
                    return accumulator;
                }, {})
            );
        } catch (error) {
    ...
    const handleDraftRoleChange = (email: string, role: UserRole) => {
    ...
    const handleUpdateUserRole = async (email: string) => {
        ...
        try {
            const { error } = await supabase
                .from('threed_user_roles')
                .upsert({ email, role: nextRole });

            if (error) throw error;

            setUsers((currentUsers) =>
                currentUsers.map((entry) =>
                    entry.email === email ? { ...entry, role: nextRole } : entry
                )
            );
            setMessage({ text: `Updated ${email} to ${nextRole}.`, type: 'success' });
        } catch (err: any) {
    ...
    const handleDeleteProject = async (projectId: string) => {
        ...
        try {
            // First delete any associated assignments to prevent foreign key errors
            await supabase
                .from('threed_outsource_assignments')
                .delete()
            ...
            alert(`Failed to delete project: ${err.message || err.details || 'Unknown error'}`);
        }
    };
                                        ...
                                        <tr key={u.email} className="hover:bg-zinc-50 dark:hover:bg-zinc-800/30 transition-colors">
                                            <td className="px-6 py-4 text-zinc-900 dark:text-white">
                                                <div className="flex items-center gap-2">
                                                    <span>{u.email}</span>
                                                    {user?.email === u.email && (
                                                        <span className="inline-flex items-center rounded-full bg-zinc-200 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-[0.14em] text-zinc-700 dark:bg-zinc-800 dark:text-zinc-300">
                                                            You
                                                        </span>
                                                    )}
                                                </div>
                                            ...
                                            <td className="px-6 py-4">
                                                <select
                                                    value={draftRoles[u.email] || u.role}
                                                    onChange={(event) => handleDraftRoleChange(u.email, event.target.value as UserRole)}
                                                    disabled={user?.email === u.email}
                                                    className="w-full max-w-[180px] bg-zinc-50 dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-lg px-3 py-2 text-sm text-zinc-900 dark:text-white outline-none focus:border-purple-500 transition-colors disabled:opacity-60 disabled:cursor-not-allowed"
                                                >
                                                    <option value="member">Member</option>
                                                    <option value="leader">Leader</option>
                                                    <option value="outsource">Outsource</option>
                                            ...
                                            <td className="px-6 py-4">
                                                <div className="flex items-center justify-end gap-2">
                                                    <button
                                                        ...
                                                        disabled={user?.email === u.email || draftRoles[u.email] === u.role || savingEmail === u.email}
                                                        className="inline-flex items-center gap-2 rounded-lg bg-purple-600 px-3 py-2 text-xs font-semibold text-white transition-colors hover:bg-purple-700 disabled:cursor-not-allowed disabled:bg-zinc-300 disabled:text-zinc-500 dark:disabled:bg-zinc-800 dark:disabled:text-zinc-500"
                                                        title="Save role"
                                                    >
                                                        <Check size={14} />
                                                    ...
                                                    <button
                                                        ...
                                                        disabled={user?.email === u.email}
                                                        className="text-zinc-400 hover:text-red-500 transition-colors p-2 disabled:cursor-not-allowed disabled:opacity-50"
                                                        title={user?.email === u.email ? 'You cannot remove your own access here' : 'Remove access'}
                                                    >
                                                        <Trash2 size={16} />
                                                    </button>
                                                </div>
                            ...
                            projects.map(project => (
                                <div key={project.id} className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl p-6">
                                    <div className="flex justify-between items-start mb-4">
                                        <div>
                                            <h4 className="text-xl font-bold text-zinc-900 dark:text-white">{project.name}</h4>
                                    ...
                                    <Tabs defaultValue="assignments" className="mt-4">
                                        <TabsList className="bg-zinc-100 dark:bg-zinc-950/50 p-1 rounded-lg mb-2 border border-zinc-200 dark:border-zinc-800">
                                            <TabsTrigger value="assignments" className="data-[state=active]:bg-white dark:data-[state=active]:bg-zinc-800 rounded-md">Assignments</TabsTrigger>
                                            <TabsTrigger value="resources" className="data-[state=active]:bg-white dark:data-[state=active]:bg-zinc-800 rounded-md">Resources</TabsTrigger>
                                            <TabsTrigger value="submissions" className="data-[state=active]:bg-white dark:data-[state=active]:bg-zinc-800 rounded-md">Submissions</TabsTrigger>
                                        </TabsList>

                                        <TabsContent value="assignments" className="border-t border-zinc-200 dark:border-zinc-800 pt-4 m-0">
                                            <h5 className="text-sm font-semibold mb-3">Assigned Outsource Workers</h5>
                                            <div className="flex flex-wrap gap-2 mb-4">
                                                {project.assignments.map((assignment: any) => (
                                                    <span key={assignment.id} className="inline-flex items-center gap-2 bg-zinc-100 dark:bg-zinc-800 text-zinc-700 dark:text-zinc-300 px-3 py-1 rounded-full text-sm">
                                                        {assignment.email}
                                                ...
                                                {project.assignments.length === 0 && <span className="text-xs text-zinc-500 italic">No one assigned</span>}
                                            </div>

                                            <div className="flex gap-2">
                                                <select
                                                    id={`assign-${project.id}`}
                                                    className="flex-1 bg-zinc-50 dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded px-3 py-2 text-sm text-zinc-900 dark:text-white outline-none focus:border-orange-500 appearance-none"
                                                >
                                                    <option value="" disabled selected>Select outsource worker...</option>
                                                    {users.filter(u => u.role === 'outsource').map(u => (
                                                        <option key={u.email} value={u.email}>{u.email}</option>
                                                    ))}
                                                </select>
                                                <button
                                                    onClick={() => {
                                                        const el = document.getElementById(`assign-${project.id}`) as HTMLSelectElement;
                                                        if (el.value) {
                                                            handleAssignOutsource(project.id, el.value);
                                                            el.value = ''; // Reset selection
                                                        }
                                                    }}
                                                    className="bg-zinc-200 hover:bg-zinc-300 dark:bg-zinc-800 dark:hover:bg-zinc-700 text-zinc-900 dark:text-white px-4 py-2 rounded text-sm transition-colors"
                                                >
                                                    Assign
                                                </button>
                                            </div>
                                        ...
                                        <TabsContent value="resources" className="border-t border-zinc-200 dark:border-zinc-800 pt-4 m-0">
                                            <div className="h-[400px] flex flex-col md:flex-row gap-4">
                                                <div className="flex-1 border border-zinc-200 dark:border-zinc-800 rounded-xl overflow-hidden bg-zinc-50 dark:bg-zinc-950/50">
                                                    <ResourceViewer folderId={project.resource_folder_id} />
                                                </div>
                                                <div className="flex-1 overflow-hidden">
                                                    <DriveUploader
                                                        folderId={project.resource_folder_id}
                                                        title="Upload Resources"
                                                        description="Upload reference files to be shared with assigned Outsource workers."
                                                    />
                                                </div>
                                            </div>
                                        </TabsContent>

                                        <TabsContent value="submissions" className="border-t border-zinc-200 dark:border-zinc-800 pt-4 m-0">
                                            <div className="h-[400px] border border-zinc-200 dark:border-zinc-800 rounded-xl overflow-hidden bg-zinc-50 dark:bg-zinc-950/50 relative">
                                                <div className="absolute top-4 left-4 z-10 bg-zinc-900/80 backdrop-blur border border-zinc-800 text-white px-3 py-1.5 rounded-lg text-sm font-medium flex items-center gap-2 shadow-lg">
                                                    Submissions from Outsource Partner
                                                </div>
                                                <ResourceViewer folderId={project.outsource_folder_id} />
                                            </div>
                                        </TabsContent>
                                    </Tabs>
                                </div>
