...
const DATE_FOLDER_PATTERN = /^\d{2}-\d{2}-\d{2}$/;

const formatSubmissionDateFolderName = () => {
    const now = new Date();
...
export const SubmissionPortal: React.FC<SubmissionPortalProps> = ({ request, onClose }) => {
    ...
    const handleFileUpload = async () => {
        ...
        try {
            ...
            const dateFolderName = formatSubmissionDateFolderName();

            let targetFolderId = selectedFolder.id;
            let targetFolderName = selectedFolder.name;

            // Check if date folder exists, if not create it
            if (!DATE_FOLDER_PATTERN.test(selectedFolder.name)) {
                try {
                    ...
                    if (existingDateFolderId) {
                        ...
                        targetFolderName = dateFolderName;
                    } else {
            ...
            if (targetFolderId !== selectedFolder.id || targetFolderName !== selectedFolder.name) {
                setSelectedFolder({ id: targetFolderId, name: targetFolderName });
            }
