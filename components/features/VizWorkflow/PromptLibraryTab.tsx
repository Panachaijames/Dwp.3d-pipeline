...
export default function PromptLibraryTab({ projId }: { projId?: string }) {
    ...
    const saveName = async (id: string) => {
        ...
        if (!newName) return cancelEditing();
        ...
        try {
            const endpoint = (activeTab === 'project' && projId) ? '/api/project-prompts' : '/api/prompt-library';
            const res = await fetch(endpoint, {
                method: 'PATCH',
                headers: { 'Content-Type': 'application/json' },
                                        ...
                                        {editingId === e.id ? (
                                            <div style={{ display: 'flex', alignItems: 'center', gap: 4, marginRight: 4 }}>
                                                <input
                                                    autoFocus
                                                    className="vw-fi"
                                                    style={{ padding: '2px 6px', fontSize: 11, height: 22, width: 160 }}
                                                    value={editNameValue}
                                                    onChange={ev => setEditNameValue(ev.target.value)}
                                                    onKeyDown={ev => { if (ev.key === 'Enter') saveName(e.id); if (ev.key === 'Escape') cancelEditing(); }}
                                                    onClick={ev => ev.stopPropagation()}
                                                />
                                                <button className="vw-btn vw-btn-ok vw-btn-sm" style={{ padding: '2px 6px', height: 22 }} onClick={(ev) => { ev.stopPropagation(); saveName(e.id); }}>✓</button>
                                                <button className="vw-btn vw-btn-g vw-btn-sm" style={{ padding: '2px 6px', height: 22 }} onClick={(ev) => { ev.stopPropagation(); cancelEditing(); }}>✕</button>
                                            </div>
                                        ) : (
                                            <div style={{ display: 'flex', alignItems: 'center', gap: 4, marginRight: 4 }}>
                                                {e.name && <span style={{ fontSize: 12, fontWeight: 700, color: 'var(--tx1)' }}>{e.name}</span>}
                                                <button
                                                    style={{ background: 'none', border: 'none', cursor: 'pointer', padding: 2, color: 'var(--tx3)', display: 'flex', alignItems: 'center', opacity: 0.7 }}
                                                    onClick={(ev) => { ev.stopPropagation(); startEditing(e.id, e.name || ''); }}
