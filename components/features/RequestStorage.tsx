...
import { SubmissionPortal } from '../portals/SubmissionPortal/SubmissionPortal';
import { FileBrowser } from '../viewers/FileBrowser';
import { DriveUploader } from './DriveUploader';

const normalizeProjectKey = (value?: string | null) => value?.trim().toLowerCase() || '';
        ...
        try {
            const [{ data, error }, { data: projectData, error: projectError }] = await Promise.all([
                ...
                supabase
                    ...
                    .select('name, resource_folder_id')
                    .order('created_at', { ascending: false })
            ]);

            if (error) throw error;
            if (projectError) {
                console.warn('Error loading related 3D project folders:', projectError);
            }

            const resourceFoldersByProject = new Map<string, string>();
            (projectData || []).forEach((project: any) => {
                const projectKey = normalizeProjectKey(project.name);
                if (!projectKey || !project.resource_folder_id || resourceFoldersByProject.has(projectKey)) {
                    return;
                }

                resourceFoldersByProject.set(projectKey, project.resource_folder_id);
            });

            // Client-side mapping & filtering
            const mapped = (data || []).map((r: any) => ({
                resourceFolderId: r.resource_folder_id || resourceFoldersByProject.get(normalizeProjectKey(r.project_name)),
                resourceFolderName: r.resource_folder_name || (r.project_name ? `${r.project_name} Resources` : 'Resources'),
                id: r.id,
                    ...
                    <div className="w-full max-w-3xl bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl shadow-2xl max-h-[90vh] overflow-y-auto transform transition-all animate-in slide-in-from-bottom-4">
                        ...
                        <div className="p-8 space-y-8">
                            ...
                            <div className="flex justify-end gap-3">
                                {selectedRequest.resourceFolderId && (
                                    <button
                                        onClick={() => window.open(`https://drive.google.com/drive/folders/${selectedRequest.resourceFolderId}`, '_blank')}
                            ...
                            {/* Requester Resource Uploads */}
                            <div className="space-y-4">
                                <div className="flex items-center justify-between">
                                    <div>
                                        <h4 className="text-sm font-bold text-zinc-400 uppercase tracking-wider">Resources For 3D Team</h4>
                                        <p className="text-sm text-zinc-500 dark:text-zinc-400 mt-1">
                                            Optional. Upload models, CAD, materials, markups, or source files for the 3D team here.
                                        </p>
                                    </div>
                                    {selectedRequest.resourceFolderId && (
                                        <button
                                ...
                                {selectedRequest.resourceFolderId ? (
                                    <div className="min-h-[440px] rounded-2xl overflow-hidden border border-zinc-200 dark:border-zinc-800">
                                        <DriveUploader
                                            folderId={selectedRequest.resourceFolderId}
                                            title="Upload Resources"
                                            description="Optional. Add source files and references for the 3D team. Final deliverables should still go to the submission folder."
                                        />
                                    </div>
                                ) : (
                                    <div className="rounded-2xl border border-dashed border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-950/30 p-6 text-sm text-zinc-500 dark:text-zinc-400">
                                        This request does not have a linked resources folder yet. Requests only get a resources portal when a Google Drive destination was selected during booking.
                                    </div>
                                )}
                            </div>
                            ...
                            {/* Submission Files Browser */}
                            {selectedRequest.driveFolderId && (
                                <div className="space-y-4">
                                    <div className="flex items-center justify-between">
                                        <h4 className="text-sm font-bold text-zinc-400 uppercase tracking-wider">Submission Folder</h4>
                                        <button
                                            onClick={() => window.open(`https://drive.google.com/drive/folders/${selectedRequest.driveFolderId}`, '_blank')}
                                            className="text-xs flex items-center gap-1 text-blue-500 hover:underline"
                                        >
