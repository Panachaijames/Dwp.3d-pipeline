...
export const FileBrowser: React.FC<FileBrowserProps> = ({ initialFolderId, accessToken, rootName = 'Project Folder' }) => {
    ...
    // Memoized Masonry Items
    const masonryItems = useMemo<MasonryItem[]>(() => {
        return browserFiles
            .filter(f => f.mimeType !== 'application/vnd.google-apps.folder') // Exclude folders for Masonry
            .map(file => {
    ...
    // Memoized Bounce Images
    const bounceImages = useMemo<string[]>(() => {
        return browserFiles
            .filter(f => /\.(jpg|jpeg|png|webp)$/i.test(f.name) && f.thumbnailLink)
