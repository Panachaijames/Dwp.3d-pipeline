...
import { OBJLoader, TDSLoader } from 'three-stdlib';
...
const ModelInner: FC<ModelInnerProps> = ({
    ...
    // Use fileName (if provided) to determine extension, since blob: URLs have no extension
    const ext = useMemo(() => {
    ...
    const content = useMemo<THREE.Object3D | null>(() => {
        ...
        if (ext === 'max' || ext === '3ds') return useLoader(TDSLoader, url).clone();
...
const ModelViewer: FC<ViewerProps> = ({
    ...
    // Determine extension to conditionally render APSViewer for .max files
    const ext = useMemo(() => {
        const name = fileName || url;
        return name.split('.').pop()!.toLowerCase();
    }, [url, fileName]);
    ...
    if (ext === 'max') {
        ...
        return (
            ...
            >
                {/* APS Viewer for .max files */}
                {/* Note: url must be a URN for APS Viewer to work, or we implement translation logic */}
                <APSViewer urn={url} getToken={getToken} />
            </div>
        );
    }
